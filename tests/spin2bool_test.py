...
def test_spin2bool(bqp_file):
    ...
    assert(isclose(data_0['offset'], data_2['offset']))

    for i in range(len(data_0['linear_terms'])):
        assert(isclose(data_0['linear_terms'][i]['coeff'], data_2['linear_terms'][i]['coeff']))

    for i in range(len(data_0['quadratic_terms'])):
        assert(isclose(data_0['quadratic_terms'][i]['coeff'], data_2['quadratic_terms'][i]['coeff']))
